...
ANIMACIÓN INICIAL
=============================================*/
//El producto aparece y baja lentamente
...
MOVIMIENTO SLIDE
=============================================*/
//El numero de la paginacion item entra como argumento y le quita 100 para que pase a la siguiente foto
function movimientoSlide(item){
    ...
    //Animacion del producto al moveerse el slide
    $(imgProducto[item]).animate({"top":-10 +"%", "opacity": 0},100)
	$(imgProducto[item]).animate({"top":30 +"px", "opacity": 1},600)
    $(titulos1[item]).animate({"top":-10+"%", "opacity":0},100)
    $(titulos1[item]).animate({"top":30+"px", "opacity":1},600)
...
setInterval(function(){
    ...
    //Aqui evitamos que el slide siga automaticamente cuando pulses al botn
	if(interrumpirCiclo){
        ...
        //So detenerintervalo es falso avanza
		if(!detenerIntervalo){
...
APARECER FLECHAS
=============================================*/
//Pasar el raton por encima del slide
$("#slide").mouseover(function(){

	$("#slide #retroceder").css({"opacity":1})
	$("#slide #avanzar").css({"opacity":1})
    //Detiene el slide automatico cuando pasa por encima
	detenerIntervalo = true;

})

//Cuando el raton sale del slide
...
$("#btnSlide").click(function(){
	...
	if(!toogle){
		...
		toogle = true;
		$("#slide").slideUp("fast");
		$("#btnSlide").html('<i class="fa fa-angle-down"></i>')//icono flecha arriba
	
	}else{

		toogle = false;
		$("#slide").slideDown("fast");
		$("#btnSlide").html('<i class="fa fa-angle-up"></i>')//Cambia icono flecha
	}
})